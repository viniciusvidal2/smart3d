...
class OpenVRDeletePointsWidget;
class OpenVRElevationWidget;
...
class InteractorStyleVR : public vtkInteractorStyle3D
{
public:
  static InteractorStyleVR *New();
  vtkTypeMacro(InteractorStyleVR, vtkInteractorStyle3D);
  ...
  */
  ...
  virtual void StartRotate(vtkEventDataDevice3D *);
  virtual void EndRotate(vtkEventDataDevice3D *);
  //@}
  ...
  */
  ...
  void ShowBillboard(const std::string &text, double r, double g, double b, int size);
  void HideBillboard();
...
protected:
  ...
  */
  ...
  //Rotate camera
  void rotateCamera(vtkEventData* ed);
  bool rotatePositive = true;
  void changeAxisRotate();
  //0 - X 1 - Y 2 - Z
  int axisToRotate = 2;
  int controlViewUpError = 0;

  double dollyPhysicalSpeedInitial = -1;
  int stepsDollyFactor = 1;
  void changeVelocity();

  //Tools
  vtkSmartPointer<OpenVRDeletePointsWidget> deletePointsWidget = NULL;
  vtkSmartPointer<OpenVRElevationWidget> elevationWidget = NULL;

  void OnWidgetDeletePoints();
  void OnWidgetElevation();

  void disableWidgets(vtkSmartPointer<vtkAbstractWidget> widgetToAvoid);

  void SetPhysicalToWorldMatrix(vtkMatrix4x4* matrix);
  void GetPhysicalToWorldMatrix(vtkMatrix4x4* physicalToWorldMatrix);

private:
  InteractorStyleVR(const InteractorStyleVR&) = delete;  // Not implemented.
  void operator=(const InteractorStyleVR&) = delete;  // Not implemented.
};
