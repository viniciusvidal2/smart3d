...
vtkStandardNewMacro(captionActor2D);

vtkCxxSetObjectMacro(captionActor2D, CaptionTextProperty, vtkTextProperty);

class captionActor2DConnection : public vtkAlgorithm
{
public:
  static captionActor2DConnection *New();
  vtkTypeMacro(captionActor2DConnection, vtkAlgorithm);

  captionActor2DConnection()
  {
    this->SetNumberOfInputPorts(1);
  }
};

vtkStandardNewMacro(captionActor2DConnection);

//----------------------------------------------------------------------------
captionActor2D::captionActor2D()
{
  ...
  this->MinimumLeaderGlyphSize = 1;

  this->LeaderGlyphConnectionHolder = captionActor2DConnection::New();
...
//----------------------------------------------------------------------------
void captionActor2D::SetLeaderGlyphConnection(vtkAlgorithmOutput* ao)
{
  this->LeaderGlyphConnectionHolder->SetInputConnection(ao);
}

//----------------------------------------------------------------------------
void captionActor2D::SetLeaderGlyphData(vtkPolyData* leader)
{
...
//----------------------------------------------------------------------------
vtkPolyData* captionActor2D::GetLeaderGlyph()
{
...
//----------------------------------------------------------------------------
void captionActor2D::SetCaption(const char* caption)
{
  this->TextActor->SetInput(caption);
}

//----------------------------------------------------------------------------
char* captionActor2D::GetCaption()
{
...
void captionActor2D::ReleaseGraphicsResources(vtkWindow *win)
{
...
int captionActor2D::RenderOverlay(vtkViewport *viewport)
{
...
//----------------------------------------------------------------------------
int captionActor2D::RenderOpaqueGeometry(vtkViewport *viewport)
{
  ...
  if (this->GetLeaderGlyph())
  {
    ...
    numPixels = (numPixels < this->MinimumLeaderGlyphSize ?
      this->MinimumLeaderGlyphSize : numPixels);
...
int captionActor2D::HasTranslucentPolygonalGeometry()
{
  return 0;
}

//----------------------------------------------------------------------------
void captionActor2D::PrintSelf(ostream& os, vtkIndent indent)
{
...
//----------------------------------------------------------------------------
void captionActor2D::ShallowCopy(vtkProp *prop)
{
  captionActor2D *a = captionActor2D::SafeDownCast(prop);
  if (a != nullptr)
  {
    ...
    this->SetMinimumLeaderGlyphSize(a->GetMinimumLeaderGlyphSize());
