...
// Krzysztof Czainski <1czajnik@gmail.com> adding:
#if wxCHECK_VERSION(2, 9, 4) // works for 2.9.4; don't know about lower 2.9.*

#  include <gtk/gtkwidget.h>
#  define wxVTK_GET_HANDLE_HACK_HELPER(wxwin) GDK_WINDOW_XWINDOW( wxwin->GetHandle()->window )
...
#  define wxVTK_GET_HANDLE_HACK_HELPER(wxwin) (wxwin)->m_wxwindow ? \
        GDK_WINDOW_XWINDOW(((GtkWidget*)piz(wxwin))->window) : \
        GDK_WINDOW_XWINDOW((wxwin)->m_widget->window)

#else

#  if wxCHECK_VERSION(2, 8, 0) && !defined(__WXGTK20__)
...
#define wxVTK_GET_HANDLE_HACK_HELPER(wxwin)   ((Window)(wxwin)->GetHandle())
#endif
...
wxVTKRenderWindowInteractor::wxVTKRenderWindowInteractor()
	...
	, UseCaptureMouse(0)
#if defined(__WXGTK__) && defined(USE_WXGLCANVAS) && wxCHECK_VERSION(2, 9, 0)
	, glContext(this)
#endif
{
...
long wxVTKRenderWindowInteractor::GetHandleHack()
{
	...
	return (long)wxVTK_GET_HANDLE_HACK_HELPER(this);
#endif
	//#ifdef __WXMOTIF__
	//    handle_tmp = (long)this->wxVTK_GET_HANDLE_HACK_HELPER();
	//#endif
...
void wxVTKRenderWindowInteractor::OnLeave(wxMouseEvent &event)
{
  ...
  if(ActiveButton != wxEVT_NULL)
  {
    ...
    mouseEvent.SetEventType(ActiveButton + 1);
    OnButtonUp(mouseEvent);
  }
