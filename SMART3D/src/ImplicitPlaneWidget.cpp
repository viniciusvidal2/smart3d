...
vtkStandardNewMacro(ImplicitPlaneWidget);

//----------------------------------------------------------------------------
ImplicitPlaneWidget::ImplicitPlaneWidget() : vtkPolyDataSourceWidget()
{
  this->DiagonalRatio = 0.3;
  this->State = ImplicitPlaneWidget::Start;
  this->EventCallbackCommand->SetCallback(ImplicitPlaneWidget::ProcessEvents);
...
//----------------------------------------------------------------------------
ImplicitPlaneWidget::~ImplicitPlaneWidget()
{
  this->Plane->Delete();
  this->Box->Delete();
  this->Outline->Delete();
...
//----------------------------------------------------------------------------
void ImplicitPlaneWidget::SetEnabled(int enabling)
{
  if (!this->Interactor)
  {
...
//----------------------------------------------------------------------------
void ImplicitPlaneWidget::ProcessEvents(vtkObject* vtkNotUsed(object),
  unsigned long event,
  void* clientdata,
  void* vtkNotUsed(calldata))
{
  ImplicitPlaneWidget* self =
    reinterpret_cast<ImplicitPlaneWidget *>(clientdata);
...
//----------------------------------------------------------------------------
void ImplicitPlaneWidget::PrintSelf(ostream& os, vtkIndent indent)
{
...
//----------------------------------------------------------------------------
void ImplicitPlaneWidget::HighlightNormal(int highlight)
{
  if (highlight)
  {
...
//----------------------------------------------------------------------------
void ImplicitPlaneWidget::HighlightPlane(int highlight)
{
  if (highlight)
  {
...
//----------------------------------------------------------------------------
void ImplicitPlaneWidget::HighlightOutline(int highlight)
{
  if (highlight)
  {
...
//----------------------------------------------------------------------------
void ImplicitPlaneWidget::OnLeftButtonDown()
{
  ...
  {
    this->State = ImplicitPlaneWidget::Outside;
    return;
  }
    ...
    prop == this->ConeActor2 || prop == this->LineActor2)
  {
    ...
    this->State = ImplicitPlaneWidget::Rotating;
  }
  ...
  else if (prop == this->SphereActor)
  {
    ...
    {
      this->HighlightNormal(1);
      this->State = ImplicitPlaneWidget::MovingOrigin;
    }
    else
    {
      return;
  ...
  else
  {
    if (this->OutlineTranslation)
    {
      ...
      this->State = ImplicitPlaneWidget::MovingOutline;
    }
    else
    {
      return;
...
//----------------------------------------------------------------------------
void ImplicitPlaneWidget::OnLeftButtonUp()
{
  if (this->State == ImplicitPlaneWidget::Outside)
  {
    return;
  }
...
//----------------------------------------------------------------------------
void ImplicitPlaneWidget::OnRightButtonDown()
{
  int X = this->Interactor->GetEventPosition()[0];
  int Y = this->Interactor->GetEventPosition()[1];
  ...
  this->ValidPick = 1;
  this->Picker->GetPickPosition(this->LastPickPosition);
  this->State = ImplicitPlaneWidget::MovingPlane;
  this->HighlightNormal(1);
  this->HighlightPlane(1);
...
//----------------------------------------------------------------------------
void ImplicitPlaneWidget::OnRightButtonUp()
{
  if (this->State == ImplicitPlaneWidget::Outside)
  {
    return;
...
//----------------------------------------------------------------------------
void ImplicitPlaneWidget::OnMouseMove()
{
  ...
  // Process the motion
  if (this->State == ImplicitPlaneWidget::MovingPlane)
  {
    this->TranslatePlane(prevPickPoint, pickPoint);
  }
  else if (this->State == ImplicitPlaneWidget::MovingOutline)
  {
    this->TranslateOutline(prevPickPoint, pickPoint);
  }
  else if (this->State == ImplicitPlaneWidget::MovingOrigin)
  {
    this->TranslateOrigin(prevPickPoint, pickPoint);
  }
  else if (this->State == ImplicitPlaneWidget::Pushing)
  {
    this->Push(prevPickPoint, pickPoint);
  }
  else if (this->State == ImplicitPlaneWidget::Scaling)
  {
    this->Scale(prevPickPoint, pickPoint, X, Y);
  }
  else if (this->State == ImplicitPlaneWidget::Rotating)
  {
...
//----------------------------------------------------------------------------
void ImplicitPlaneWidget::OnMouseWheelForward()
{
  this->SizeHandles();
}
//----------------------------------------------------------------------------
void ImplicitPlaneWidget::OnMouseWheelBackward()
{
  this->SizeHandles();
}
//----------------------------------------------------------------------------
void ImplicitPlaneWidget::OnKeyPress()
{
  char key = this->Interactor->GetKeyCode();
  if (key == '=')
  {
    this->SetDiagonalRatio(this->GetDiagonalRatio() + 0.01);
    this->UpdateRepresentation();
    this->Interactor->Render();
  }
  else if (key == '-')
  ...
  {
    if (this->GetNormalToXAxis())
    {
      this->SetNormalToXAxis(false);
      this->Plane->SetNormal(-1, 0, 0);
      this->LineSource->SetPoint2(-1, 0, 0);
    }
    else
    {
      this->SetNormalToXAxis(true);
      this->Plane->SetNormal(1, 0, 0);
      this->LineSource->SetPoint2(1, 0, 0);
    }
    this->UpdateRepresentation();
  ...
  {
    if (this->GetNormalToYAxis())
    {
      this->SetNormalToYAxis(false);
      this->Plane->SetNormal(0, -1, 0);
      this->LineSource->SetPoint2(0, -1, 0);
    }
    else
    {
      this->SetNormalToYAxis(true);
      this->Plane->SetNormal(0, 1, 0);
      this->LineSource->SetPoint2(0, 1, 0);
    }
    this->UpdateRepresentation();
  ...
  {
    if (this->GetNormalToZAxis())
    {
      this->SetNormalToZAxis(false);
      this->Plane->SetNormal(0, 0, -1);
      this->LineSource->SetPoint2(0, 0, -1);
    }
    else
    {
      this->SetNormalToZAxis(true);
      this->Plane->SetNormal(0, 0, 1);
      this->LineSource->SetPoint2(0, 0, 1);
    }
    this->UpdateRepresentation();
...
//----------------------------------------------------------------------------
void ImplicitPlaneWidget::Rotate(int X, int Y, double *p1, double *p2, double *vpn)
{
...
void ImplicitPlaneWidget::TranslatePlane(double *p1, double *p2)
{
...
//----------------------------------------------------------------------------
// Loop through all points and translate them
void ImplicitPlaneWidget::TranslateOutline(double *p1, double *p2)
{
  //Get the motion vector
  double v[3];
  v[0] = p2[0] - p1[0];
...
//----------------------------------------------------------------------------
// Loop through all points and translate them
void ImplicitPlaneWidget::TranslateOrigin(double *p1, double *p2)
{
  //Get the motion vector
  double v[3];
  v[0] = p2[0] - p1[0];
...
//----------------------------------------------------------------------------
void ImplicitPlaneWidget::Scale(double *p1, double *p2,
...
//----------------------------------------------------------------------------
void ImplicitPlaneWidget::Push(double *p1, double *p2)
{
  //Get the motion vector
  double v[3];
  v[0] = p2[0] - p1[0];
...
//----------------------------------------------------------------------------
void ImplicitPlaneWidget::CreateDefaultProperties()
{
  // Normal properties
...
//------------------------------------------------------------------------------
void ImplicitPlaneWidget::RegisterPickers()
{
  this->Interactor->GetPickingManager()->AddPicker(this->Picker, this);
}

//----------------------------------------------------------------------------
void ImplicitPlaneWidget::PlaceWidget(double bds[6])
{
  int i;
...
void ImplicitPlaneWidget::SetOrigin(double x, double y, double z)
{
  double origin[3];
  origin[0] = x;
  origin[1] = y;
...
// Description:
// Set the origin of the plane.
void ImplicitPlaneWidget::SetOrigin(double x[3])
{
...
double* ImplicitPlaneWidget::GetOrigin()
{
  return this->Plane->GetOrigin();
}

//----------------------------------------------------------------------------
void ImplicitPlaneWidget::GetOrigin(double xyz[3])
{
...
void ImplicitPlaneWidget::SetNormal(double x, double y, double z)
{
  double n[3];
  n[0] = x;
  n[1] = y;
...
// Description:
// Set the normal to the plane.
void ImplicitPlaneWidget::SetNormal(double n[3])
{
...
double* ImplicitPlaneWidget::GetNormal()
{
  return this->Plane->GetNormal();
}

//----------------------------------------------------------------------------
void ImplicitPlaneWidget::GetNormal(double xyz[3])
{
  this->Plane->GetNormal(xyz);
}

vtkActor* ImplicitPlaneWidget::GetPlaneActor()
{
  return this->CutActor;
}

//----------------------------------------------------------------------------
void ImplicitPlaneWidget::SetDrawPlane(int drawPlane)
{
...
//----------------------------------------------------------------------------
void ImplicitPlaneWidget::SetNormalToXAxis(int var)
{
...
//----------------------------------------------------------------------------
void ImplicitPlaneWidget::SetNormalToYAxis(int var)
{
...
//----------------------------------------------------------------------------
void ImplicitPlaneWidget::SetNormalToZAxis(int var)
{
...
//----------------------------------------------------------------------------
void ImplicitPlaneWidget::GetPolyData(vtkPolyData *pd)
{
  pd->ShallowCopy(this->Cutter->GetOutput());
}

//----------------------------------------------------------------------------
vtkPolyDataAlgorithm *ImplicitPlaneWidget::GetPolyDataAlgorithm()
{
  return this->Cutter;
}

//----------------------------------------------------------------------------
void ImplicitPlaneWidget::GetPlane(vtkPlane *plane)
{
  if (plane == NULL)
  {
    return;
...
//----------------------------------------------------------------------------
void ImplicitPlaneWidget::UpdatePlacement()
{
  this->Outline->Update();
  this->Cutter->Update();
  this->Edges->Update();
...
//----------------------------------------------------------------------------
void ImplicitPlaneWidget::UpdateRepresentation()
{
  if (!this->CurrentRenderer)
  {
    return;
...
//----------------------------------------------------------------------------
void ImplicitPlaneWidget::SizeHandles()
{
